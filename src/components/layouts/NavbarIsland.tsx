...
import { type Lang, languages } from "@i18n/config.ts";
...
export default function NavbarIsland({ lang, route }: Props) {
  const t = useTranslations(lang);
  const otherLangs = (Object.keys(languages) as Lang[]).filter(l => l !== lang);

  const links = [
    { href: getPath(lang, ""), label: t("NavbarIsland.index") },
    { href: getPath(lang, "bases"), label: t("NavbarIsland.regulations") },
    { href: getPath(lang, "premios"), label: t("NavbarIsland.prizes") },
    { href: getPath(lang, "jugadores"), label: t("NavbarIsland.players") },
    { href: getPath(lang, "partidas"), label: t("NavbarIsland.games") },
    { href: getPath(lang, "alojamiento"), label: t("NavbarIsland.accommodation") },
    { href: getPath(lang, "fotos"), label: t("NavbarIsland.pictures") },
    { href: getPath(lang, "noticias"), label: t("NavbarIsland.news") },
    { href: getPath(lang, "contacto"), label: t("NavbarIsland.contact") },
  ];
  ...
  return (
    <>
      <div class="flex justify-end pt-6">
        <LangSwitcher lang={lang} route={route} otherLangs={otherLangs} />
      </div>

      <nav class="flex justify-between items-center gap-8 pt-2 pb-12">
        {/* Logo */}
        <a href={getPath(lang, "")} class="flex items-center gap-2 flex-shrink-0">
          <svg
            version="1.1"
          ...
          <div>
            <span class="block text-blue-900 text-base">{t("NavbarIsland.tournamentName")}</span>
        ...
        <div ref={ref} class="relative flex-shrink-0">
          <button
            ...
            aria-label={t("NavbarIsland.openMenu")}
            aria-expanded={open}
          >
            {open ? (
...
function LangSwitcher({ lang, route, otherLangs }: LangSwitcherProps) {
  return (
    <div class="flex items-center gap-2 text-xs uppercase tracking-widest">
      <span class="font-bold text-blue-900">{lang}</span>
      {otherLangs.map((l) => (
        <>
          <span class="w-[1px] h-3 bg-gray-200 inline-block" aria-hidden="true" />
          <a href={getPath(l, route)} class="link">{l}</a>
        </>
      ))}
    </div>
  );
