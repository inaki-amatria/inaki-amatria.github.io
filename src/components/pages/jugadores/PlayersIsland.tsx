...
export default function PlayersIsland({ lang }: Props) {
  ...
  return (
    <div class="flex flex-col gap-4">
      <div class="flex gap-4">
        ...
        >
          {t('PlayersIsland.groupA')}
        </button>
        <button
          onClick={() => setGroup("amateur")}
          class={`cursor-pointer ${group === "amateur" ? "button-primary" : "button-secondary"}`}
        >
          {t('PlayersIsland.groupB')}
        </button>
      </div>

      <div class="card overflow-hidden">
        <div class="hidden sm:grid grid-cols-[3rem_4rem_1fr_5rem_4rem] table-header">
          <span class="table-header-cell">{t('PlayersIsland.rank')}</span>
          <span class="table-header-cell">{t('PlayersIsland.title')}</span>
          <span class="table-header-cell">{t('PlayersIsland.name')}</span>
          <span class="table-header-cell">{t('PlayersIsland.elo')}</span>
          <span class="table-header-cell">{t('PlayersIsland.fed')}</span>
        </div>
        ...
        {error && (
          <div class="px-6 py-3 text-gray-600 text-sm">
            {t('PlayersIsland.error')}
          </div>
        )}

        {!loading && !error && players.length === 0 && (
          <div class="px-6 py-3 text-gray-600 text-sm">
            {t('PlayersIsland.empty')}
          </div>
        )}
