...
export default function MapIsland({ lang, src, class: className = "", iframeClass = "w-full h-full" }: Props) {
    ...
    <div class={`overflow-hidden bg-blue-50 rounded-3xl shadow-lg ${className}`}>
      {consent === "accepted" ? (
        <iframe
          ...
          class={iframeClass}
          style="border:0;"
          title={t("MapIsland.acceptedTitle")}
          allowFullScreen
          loading="lazy"
        ...
        <div class="w-full h-full min-h-48 bg-blue-50 flex flex-col items-center justify-center gap-4 p-6 text-center">
          ...
          <div class="flex flex-col gap-2">
            <p class="text-blue-900 font-bold">{t("MapIsland.declinedTitle")}</p>
            <p class="text-gray-600 text-sm max-w-xs">
              {t("MapIsland.declinedSubtitle")}
            </p>
          </div>
        </div>
      )}
